...
const cities = `[
  ...
  let weather = {
    apiKey: "ea8777f7cd299604a7082b4974bd8d7d",
    ...
    displayWeather: function (data) {
      ...
      document.querySelector(".temp").innerText = celcius + " °C";
      const link = document.querySelector("#link");
      if (celcius > 17) {
        document.querySelector("#place").innerText =
          "It seems to be hot out here. You can visit places like: St. Moritz (Switzerland), Lyon (France), Mt. Cayambe (Ecuador), etc.";
        link.innerText = "For more suggestion, click here!";
        link.setAttribute(
          "href",
          "https://timesofindia.indiatimes.com/travel/destinations/cold-places-to-visit-in-india-to-beat-the-heat-during-summers/photostory/84064167.cms?picid=84064208"
        );
      } else {
        document.querySelector("#place").innerText =
          "It seems to be cool out here. You can visit places like: Goa (India), Cape Town (South Africa), Dubai (UAE), etc.";
        link.innerText = "For more suggestion, click here!";
        link.setAttribute(
          "href",
          "https://www.tourism-of-india.com/blog/warmest-places-in-india-to-visit-this-winter/"
        );
      }
      ...
      document.querySelector("#suggest1").innerText = "";
      document.querySelector("#suggest2").innerText = "";
      document.querySelector("#suggest3").innerText = "";
    },
    search: function () {
      this.fetchWeather(document.querySelector(".search-bar").value);
    },
    find: function (initials) {
      let counter = 0;
      let ans = ["", "", ""];
      for (let city of cityData) {
        if (city.city.toLowerCase().startsWith(initials.toLowerCase())) {
          counter++;
          ans[counter - 1] = city.city;
        }
        if (counter === 3) {
          break;
        }
      }
      document.querySelector("#suggest1").innerText = ans[0];
      document.querySelector("#suggest2").innerText = ans[1];
      document.querySelector("#suggest3").innerText = ans[2];
    },
    changeText: function (city) {
  ...
  document
    .querySelector(".search-bar")
    .addEventListener("keyup", function (event) {
      weather.find(document.querySelector(".search-bar").value);
    });
  
  document.querySelector("#suggest1").addEventListener("click", function (event) {
    // console.log(event.srcElement.firstChild.data);
    weather.changeText(event.srcElement.firstChild.data);
    // console.log(document.querySelectorAll(".search-bar").innerText)
    // document.querySelectorAll(".search-bar").innerText=event.srcElement.firstChild.data;
  });
  
  document.querySelector("#suggest2").addEventListener("click", function (event) {
    weather.changeText(event.srcElement.firstChild.data);
  });
  
  document.querySelector("#suggest3").addEventListener("click", function (event) {
    weather.changeText(event.srcElement.firstChild.data);
  });
  
  //https://api.openweathermap.org/data/2.5/weather?q=denver&appid=ea8777f7cd299604a7082b4974bd8d7d
